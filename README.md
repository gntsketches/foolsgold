...
and re-renders must be managed directly (via the `clearAndRender` method).
...
or `classNames` (array of strings). Above, the `div` receives the class "main".
...
The `Counter` component needs to call the `clearAndRender` method in its constructor.
Without this the `DOMCount` displays `undefined` as the first pass of "render" is called in the parent constructor.
`addToCount` also uses the `clearAndRender` method.
...
## Store and State
Foolsgold provides a basic system for centralized state management throught the store.
To use it, it's necessary to pass in a special `initialState` prop during instantiation of the App at the top level.
...
```
The `initialState` is defined with a field for the list items,
and the `List` component iterates over them to generate the elements.
The List registers to `addToList` , which is published
by the click handler on the `DOMAdd` element, so the list re-renders 
at that time. The `DOMInput` is assigned as a property for the 
App component, so it can be accessed in the `addToList` method to clear
the input field.
